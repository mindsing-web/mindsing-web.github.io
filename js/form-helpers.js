...
(function () {
  ...
  function computeDebtValue(form) {
    try {
      ...
      console.error('computeDebtValue error:', e);
      return 0;
    }
  }
  ...
  function renderDebtOutput(form) {
    try {
      ...
      // Build markup: bold D= then formatted currency
      out.innerHTML = '<p class="mb0"><strong>D=</strong> ' + formatCurrency(value) + '</p>';
    } catch (e) {
      console.error('renderDebtOutput error:', e);
    }
  }
  ...
  function initDebtOutput(root) {
    ...
    try {
      ...
      // Initial render (after persistence restore)
      renderDebtOutput(form);

      // Update on blur and change for the debt inputs
      var ids = ['final_expenses', 'credit_card_debts', 'car_loans', 'other_debts'];
      ids.forEach(function (id) {
        var el = form.querySelector('#' + id);
        if (!el) return;
      ...
      console.error('initDebtOutput error:', e);
    }
  }
